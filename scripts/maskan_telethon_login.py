"""One-time interactive Telethon login for the Maskan client userbot.

Prompts for the SMS code Telegram sends to MASKAN_USERBOT_PHONE (and the 2FA
password if set). Writes the session to MASKAN_USERBOT_SESSION; after that the
...
from apps.maskan.config import config  # noqa: E402


async def main() -> None:
    if not config.api_id or not config.api_hash or not config.userbot_phone:
        print(
            "Set TG_API_ID, TG_API_HASH, and MASKAN_USERBOT_PHONE in .env first.",
            file=sys.stderr,
        )
        sys.exit(1)
